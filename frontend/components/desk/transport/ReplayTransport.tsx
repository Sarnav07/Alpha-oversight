...
import { CASES } from "@/lib/fixtures/registry";
...
/** A short, human marker tone for a frame (rail ticks + readout dot). */
function frameTone(e: ActivityEvent): keyof typeof TONE_VAR | null {
  if (/waiting on band/i.test(e.content ?? "")) return "band";
  const m = parseMarker(e);
  if (m.result === "PASS") return "pass";
  if (m.result === "FLAG") return "flag";
  if (m.stage === "escalate") return "escalate";
  if (m.stage === "codify") return "complete";
  return null;
}

/** A compact label for the frame currently under the playhead. */
function frameLabel(e: ActivityEvent | undefined): string {
  if (!e) return "ready";
  const c = e.content ?? "";
  const term = c.match(/->\s*([A-Z_]+)\s*$/);
  if (term) return term[1];
  const verdict = c.match(/verdict=(PASS|FLAG)/);
  if (verdict) return c.includes("rule=None") ? `${verdict[1]}` : `${verdict[1]} ✓rule`;
  if (/waiting on band/i.test(c)) return "waiting on Band";
  if (/debate complete/i.test(c)) return "debate complete";
  if (/^round \d/i.test(c)) return c.slice(0, 22);
  return e.agent_name;
}
...
export default function ReplayTransport() {
  const { caseId, frames, index, status, speed } = useClockStore();
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5">
        ...
        />
        <IconBtn label="Reset" glyph="↺" onClick={reset} disabled={!caseId} />
      </div>
