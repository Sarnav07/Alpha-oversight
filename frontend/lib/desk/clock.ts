...
interface ClockState {
  /** the loaded fixture's case id (null when idle). */
  caseId: string | null;
  ...
  /** load a fixture by case id; resets the trace, parks paused at frame 0. */
  ...
  append: (frames: ActivityEvent[]) => void;
  reset: () => void;
}
...
export const useClockStore = create<ClockState>((set, get) => {
  ...
  return {
    ...
    load: (caseId) => {
      ...
      set({ caseId, frames: framesFor(caseId), index: 0, status: "paused" });
    ...
    reset: () => {
      clearTicker();
      useTraceStore.getState().reset();
      set({ caseId: null, frames: [], index: 0, status: "idle" });
      syncConnection("idle");
    },
  };
});
