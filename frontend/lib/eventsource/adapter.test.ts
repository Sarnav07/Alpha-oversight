...
function makeHandlers() {
  ...
  return {
    handlers: {
      onEvent: (e: ActivityEvent) => events.push(e),
...
describe("LiveSSEAdapter (createAdapter in live mode)", () => {
  it("emits 'connecting' immediately and constructs one EventSource", () => {
    ...
    expect(instances[0].url).toContain("/stream");
  });
  ...
  it("replay opt opens with ?replay and reports 'replay' on open", () => {
    const { handlers, states } = makeHandlers();
    createAdapter({ replay: "C-0187" }).connect(handlers);

    expect(instances[0].url).toContain("replay=C-0187");
    instances[0].simulateOpen();
  ...
  it("valid JSON frame → onEvent(frame)", () => {
    ...
    const frame = ev({ agent_name: "prosecution" });
  ...
  it("desk filter drops frames from the other desk", () => {
    const { handlers, events } = makeHandlers();
    createAdapter({ desk: "rnd" }).connect(handlers);
    instances[0].simulateOpen();

    instances[0].simulateMessage(JSON.stringify(ev({ desk: "surveillance" })));
    expect(events).toHaveLength(0);

    const kept = ev({ desk: "rnd" });
